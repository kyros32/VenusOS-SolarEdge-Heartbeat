    ...
    def scan_network(self):
        ...
        try:
            ...
            for i in range(1, 255):
                if self.settings['AutoDiscover'] == 0: break 
                test_ip = f"{base_ip}.{i}"
            ...
            if client.connect():
                try:
                    ...
                    if not resp.isError():
                        ...
                        if idx == 0: ui_grid_enabled = val # Display first device's state on UI
                        
                        if val == 0:
                            client.write_registers(61762, client.convert_to_registers(1, ModbusClientMixin.DATATYPE.UINT32, word_order='little'), slave=slave_id)
                            client.write_registers(61696, client.convert_to_registers(1, ModbusClientMixin.DATATYPE.UINT16, word_order='little'), slave=slave_id)
                    ...
                    if not t_resp.isError() and not f_resp.isError():
                        curr_t = client.convert_from_registers(t_resp.registers, ModbusClientMixin.DATATYPE.UINT32, word_order='little')
                        curr_f = client.convert_from_registers(f_resp.registers, ModbusClientMixin.DATATYPE.FLOAT32, word_order='little')
                        ...
                        if curr_t != target_t:
                            client.write_registers(62224, client.convert_to_registers(target_t, ModbusClientMixin.DATATYPE.UINT32, word_order='little'), slave=slave_id)
                        if curr_f != target_f:
                            client.write_registers(62226, client.convert_to_registers(target_f, ModbusClientMixin.DATATYPE.FLOAT32, word_order='little'), slave=slave_id)
